...
class Obstacle {
  constructor(x, y, speed, img) {
    this.x = x;
    this.y = y;
    this.speed = speed / 3.5; // Замедление скорости в 2.5 раза
...
class Lane {
  constructor(y, laneType) {
    ...
    this.obstacles = [];
    if (laneType === 'road') {
      const laneSpeed = randomChoice([-3, 3]) / 3.5; // Замедление: ±3 → ±1.2
      ...
      const speed = randomChoice([-2, 2]) / 3.5; // Замедление: ±2 → ±0.8
